...
static int transfer_file(axidma_dev_t dev, struct dma_transfer *trans)
{
	...
	while(1) {
		if((reg_val = read_reg(0x43c00010)) > 0x3900){
			continue;
		}
		count += 1024;
		send_size = count > trans->input_size ? trans->input_size%1024 : 1024;
		rc = robust_read(trans->input_fd, trans->input_buf, send_size);
		 if (rc < 0) {
			perror("Unable to read in input buffer.\n");
			axidma_free(dev, trans->input_buf, trans->input_size);
			return rc;
		}
		rc = axidma_oneway_transfer(dev, trans->input_channel, trans->input_buf, send_size, true);
